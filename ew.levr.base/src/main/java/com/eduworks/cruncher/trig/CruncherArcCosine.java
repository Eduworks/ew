...
public class CruncherArcCosine extends Cruncher
{
	...
	public Object resolve(Context c, Map<String, String[]> parameters, Map<String, InputStream> dataStreams) throws JSONException
	{
		...
		if(!sDeg.isEmpty()){
			return Math.acos(Math.toRadians(Double.parseDouble(sDeg)));
		}else if(!sRad.isEmpty()){
			return Math.acos(Double.parseDouble(sRad));
		}else{
			double obj = getAsDouble("obj", c, parameters, dataStreams);
			if(obj < 2 * Math.PI && obj > -2 * Math.PI){
				return Math.acos(obj);
			}else{
				return Math.acos(Math.toRadians(obj));
			}
		}
	}
	...
	{
		return "Retrieves the ArcCosine of the angle passed in. If angle passed in via 'obj' will assume values < 2*pi are radians";
	}
