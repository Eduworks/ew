...
public class CruncherArcTangent extends Cruncher
{
	...
	public Object resolve(Context c, Map<String, String[]> parameters, Map<String, InputStream> dataStreams) throws JSONException
	{
		...
		if(!sDeg.isEmpty()){
			return Math.atan(Math.toRadians(Double.parseDouble(sDeg)));
		}else if(!sRad.isEmpty()){
			return Math.atan(Double.parseDouble(sRad));
		}else{
			double obj = getAsDouble("obj", c, parameters, dataStreams);
			if(obj < 2 * Math.PI && obj > -2 * Math.PI){
				return Math.atan(obj);
			}else{
				return Math.atan(Math.toRadians(obj));
			}
		}
	}
	...
	{
		return "Retrieves the ArcTangent of the angle passed in. If angle passed in via 'obj' will assume values < 2*pi are radians";
	}
