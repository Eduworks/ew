...
public class CruncherScoreboard extends Cruncher
{
	...
	public Object resolve(Context c, Map<String, String[]> parameters, Map<String, InputStream> dataStreams) throws JSONException
	{
		...
		Object o = get("array", c,parameters,dataStreams);
		...
		if (optAsBoolean("lookup", false, c,parameters,dataStreams))
		{
			int startPoint;
			if (has("key"))
			{
				for (JSONObject jo : wses)
					if (jo.getString("key").equals(getAsString("key", c,parameters,dataStreams)))
						representativeWS = jo;
				int focusPoint;
				...
				startPoint = Math.max(0, focusPoint - optAsInteger("aboveCount", 9, c,parameters,dataStreams));
				int maxPoint = Math.min(wses.size(), startPoint + optAsInteger("belowCount", 10, c,parameters,dataStreams));
				wses = (List<JSONObject>) wses.subList(startPoint, maxPoint);
			}
			else
			{
				startPoint = 0;
				wses = wses.subList(0, optAsInteger("belowCount", 10, c,parameters,dataStreams));
			}
		...
		else
		{
			if (has("key"))
			{
				...
				if (representativeWS == null)
				{
					...
					representativeWS.put("key",getAsString("key", c,parameters,dataStreams));
					wses.add(representativeWS);
				}
				if (has("name"))
				{
					representativeWS.put("name",getAsString("name", c,parameters,dataStreams));
				}
				if (has("score"))
				{
					representativeWS.put("score",getAsDouble("score", c,parameters,dataStreams));
				}
			}
