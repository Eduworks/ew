...
public class CruncherDisplayJson extends Cruncher
{
	...
	{
		final boolean collapse = optAsBoolean("collapse", false, c,parameters,dataStreams);
		final boolean asArray = optAsBoolean("asArray", false, c,parameters,dataStreams);
		final Integer spaces = optAsInteger("spaces", 5, c,parameters,dataStreams);
		...
		if (collapse && data.length() == 1)
			result = data.get(data.keys().next().toString());
		else
		{
			if (spaces > 0)
				result = data.toString(spaces);
			else
