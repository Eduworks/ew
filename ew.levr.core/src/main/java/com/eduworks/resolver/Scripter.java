...
public abstract class Scripter implements Resolvable
{
	...
	public String toOriginalJson() throws JSONException
	{
		...
		String func = getClass().getSimpleName().replace("Resolver", "").replace("Cruncher", "").replace("Scripter", "");
	...
	{
		return getClass().getSimpleName().replace("Scripter", "").toLowerCase().substring(0, 1)
				+ getClass().getSimpleName().replace("Scripter", "").substring(1);
	}
